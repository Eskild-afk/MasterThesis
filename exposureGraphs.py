...
if True:
    print('10Y Payer Swap with Initial Margin')
    start = timer.time()
    ...
    def worker(i):
        ...
        for t in enumerate(time):
            ...
            IM.append((np.maximum(IMta-KIM,0)>MTA)*np.maximum(IMta-KIM,0))
        D = np.cumprod(np.insert(np.exp(-float[:-1:]*dt),0,1)) 
        result = np.where(swap>0, np.maximum(swap-IM,0), np.minimum(swap+IM,0))
        print('{:.2f}%'.format(round(i/sims*100, 2)), end='\r')
        return D*np.maximum(result, 0), D*np.minimum(result, 0)
    ...
    plt.xticks(np.arange(0, 10.5, 2))
    plt.legend(frameon = False, fontsize = 18, loc='upper right')
    dump(PE/sims, f'./SimulationData/PE_10Y_Swap_With_IM_N={sims}_dt={int(1/dt)}.joblib')
    dump(NE/sims, f'./SimulationData/NE_10Y_Swap_With_IM_N={sims}_dt={int(1/dt)}.joblib')
    plt.savefig(f'./Graphs/Exposure_Plot_10YPayer_Swap_With_IM_N={sims}_dt={int(1/dt)}.png', bbox_inches='tight')
    print( 'Finished creating graph')
    cva   = CVA(time, dt, discounting*PE/sims)
    cvaUB = CVA(time, dt, HPUB)
    cvaLB = CVA(time, dt, HPLB)
    ...
    dvaLB = DVA(time, dt, HMLB)
    with open('SimulationTimes.txt', 'a') as f:
        f.write(f'\n{sims},{int(1/dt)},{cva},{cvaUB},{cvaLB},{dva},{dvaUB},{dvaLB},10Y Payer Swap with IM,{timer.time()-start:.2f}')
